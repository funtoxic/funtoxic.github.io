(_ => {
	...
	window.onload = function () {
		...
		const createAddonList = (type, ending) => {
			let [header, list] = createLoadingList(`https://funtoxic.github.io/BetterDiscordAddons/${type}/README.md`, {
				...
				getChildren: response => response.split("- [").splice(1).map(n => {
					...
					return name && (!createAddonList.search[type] || [name, description].filter(n => n).some(s => s.toLowerCase().indexOf(createAddonList.search[type].toLowerCase()) > -1)) && createAddonCard({
						...
						resourceUrl: `https://funtoxic.github.io/BetterDiscordAddons/${type}/${fileName}/_res`,
						websiteUrl: `https://github.com/funtoxic/BetterDiscordAddons/tree/master/${type}/${fileName}`,
						sourceUrl: `https://funtoxic.github.io/BetterDiscordAddons/${type}/${fileName}/${fileName}${ending}`,
		...
		const createLibraryEntry = _ => {
			return createLoadingList("https://funtoxic.github.io/BetterDiscordAddons/Library/README.md", {
				className: "addon-list",
				...
				getChildren: response => {
					...
					return createAddonCard({
						...
						resourceUrl: "https://funtoxic.github.io/BetterDiscordAddons/Library/_res",
						websiteUrl: "https://github.com/funtoxic/BetterDiscordAddons/tree/master/Library",
						sourceUrl: `https://funtoxic.github.io/BetterDiscordAddons/Library/0${fileName}.plugin.js`,
						download: "?library"
					});
				}
			});
		...
		const sections = [
			...
			{section: "General", renderSection: _ => [
				...
				createElement(`<div class="welcome-details">Here you'll find all my Plugins and Themes that are hosted on my <a href="https://github.com/funtoxic/BetterDiscordAddons/" target="_blank">GitHub</a>.</div>`),
				createElement(`<div class="welcome-details">If you expierence any Issues or got some Improvement Suggestions for my stuff, then feel free to drop them on <a href="https://github.com/funtoxic/BetterDiscordAddons/issues" target="_blank">here</a>. But please make sure whether there is already an open Thread for your Issue or Suggestion.</div>`),
				createElement(`<div class="divider"></div>`),
			...
			{section: "Invites", renderSection: _ => [
				createInvite({invite: "Jx3TjNS", name: "DevilBro's Haus", details: "Support Server for all my Stuff", icon: "https://funtoxic.github.io/_res/imgs/db_logo.webp"}),
				createElement(`<div class="divider"></div>`),
				createInvite({invite: "0Tmfo5ZbORCRqbAd", name: "BetterDiscord", details: "First Support Server for BD", icon: "https://funtoxic.github.io/_res/imgs/bd1_logo.webp"}),
				createInvite({invite: "sbA3xCJ", name: "BetterDiscord2", details: "Second Support Server for BD", icon: "https://funtoxic.github.io/_res/imgs/bd2_logo.webp"})
			]}
		];
		let items = [];
